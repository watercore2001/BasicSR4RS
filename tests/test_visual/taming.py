...
params = torch.load("experiments/pretrained_models/sen2naip256_vq_f4_dim4.pth", map_location=device)

model.load_state_dict(params)
model.eval()

visual_fodler = "tamming"
...
with torch.no_grad():
    for batch in val_dataloader:
        ...
        lq_img = batch['lq'].to(device)
        gt_img = batch['gt'].to(device)

        lq_ae = model(lq_img)
        gt_ae = model(gt_img)

        lq_img = zero_one_tensor_to_ubyte_numpy(lq_img.detach().cpu())  # numpy H(BW)(RGBNIR) uint8
        gt_img = zero_one_tensor_to_ubyte_numpy(gt_img.detach().cpu())  # numpy H(BW)(RGBNIR) uint8
        lq_ae = zero_one_tensor_to_ubyte_numpy(lq_ae.detach().cpu())  # numpy H(BW)(RGBNIR) uint8
        gt_ae = zero_one_tensor_to_ubyte_numpy(gt_ae.detach().cpu())
