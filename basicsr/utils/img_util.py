...
def save_lq_sr_image(lq_img: np.ndarray, sr_img: np.ndarray, all_path: str):
    ...
    dir_name = os.path.abspath(os.path.dirname(all_path))
    os.makedirs(dir_name, exist_ok=True)
    plt.savefig(all_path, dpi=100, bbox_inches='tight')
    plt.close('all')


def save_all_image(lq_img: np.ndarray, sr_img: np.ndarray, gt_img: np.ndarray, all_path: str):
    """
        lq_img: uin8 [0-255], HWC, Channels: BGR or single
        sr_img: uin8 [0-255], HWC, Channels: BGR or single
        gt_img: uin8 [0-255], HWC, Channels: BGR or single
